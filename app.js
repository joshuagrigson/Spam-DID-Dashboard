...
function App() {
  ...
  const [intelTab, setIntelTab] = useState('command');
  ...
  // ── Intelligence layer (analytics.js) ──────────────────────────────────────
  // Reputation is carrier ground truth imported from Ignite / CallPurity /
  // DNC.com / Caller ID Reputation. Snapshots are the per-import history that
  // makes any trend possible at all — before they existed each import destroyed
  // the last one. Declared HERE, above enrichedAll, because the pool table needs
  // the carrier verdict on every row — not just the analytics tabs.
  const [repBy, setRepBy]     = useState(() => repStore.load().byPhone);
  const [snaps, setSnaps]     = useState(() => snapStore.load().snaps);
  const [repOpen, setRepOpen] = useState(false);
  const [bfOpen, setBfOpen]   = useState(false);

  // ── Derived data ────────────────────────────────────────────────────────────
  // `rep` is attached to every row so the carrier verdict travels with the number
  // everywhere it is shown, sorted or filtered. calcScore() is deliberately NOT
  // fed from it: the score stays a pure Convoso-performance measure and the
  // carrier verdict sits beside it as a separate, independent fact.
  const enrichedAll = useMemo(() => dids.map(d => {
    const s = calcScore(d);
    const rep = repOf(d.did, repBy);
    return { ...d, score: s, grade: getGrade(s), fmt: fmtDID(d.did),
             area: areaCode(d.did), center: centerOf(d.campaign),
             rep, repRank: repRank(rep) };
  }), [dids, repBy]);
  ...
  // Diagnosis runs against the SCOPED set so a verdict always reflects the
  // center you are looking at — an area-code median mixed across two call
  // centers would compare numbers that never shared a list.
  const diagRows = useMemo(() => {
    const c = buildCtx(enriched, repBy, snaps);
    return { rows: enriched.map(d => ({ ...d, diag: classify(d, c) })), ctx: c };
  }, [enriched, repBy, snaps]);
  ...
  const filtered = useMemo(() => {
    ...
    else if (filter === 'carrier') rows = rows.filter(d => d.rep && d.rep.flagged === true);
  ...
  function applyImport() {
    ...
    // Keep this import as a dated snapshot BEFORE the pool is overwritten. This
    // one line is what turns the dashboard from a photograph into a record.
    setSnaps(snapStore.record(
      preview.map(r => ({ did: normPh(r.rawDid), calls: r.calls, cr: r.cr, dncCount: r.dncCount })),
      fname
    ).snaps);
  ...
  const TABS = [
    ...
    { k: 'carrier', l: `Carrier Flagged (${viewCounts.carrierFlagged})`, warn: viewCounts.carrierFlagged > 0 },
  ...
  function renderIntel() {
    ...
    const tabs = [
      { k: 'command', l: 'Command',     i: 'ti-radar-2'      },
      { k: 'trends',  l: 'Trends',      i: 'ti-trending-up'  },
      { k: 'rep',     l: 'Reputation',  i: 'ti-shield-check' },
      { k: 'buy',     l: 'Buy list',    i: 'ti-shopping-cart'},
    ...
    return h('div', { className: 'intel-panel' },
      ...
      // ── Intelligence tabs (analytics.js) ──
      intelTab === 'command' && h(CommandCenter, { rows: diagRows.rows, snaps, repBy }),
      intelTab === 'trends'  && h(TrendsView,    { rows: diagRows.rows, snaps,
                                                   onBackfill: () => setBfOpen(true) }),
      intelTab === 'rep'     && h(ReputationView, {
        rows: diagRows.rows, repBy,
        onImport: () => setRepOpen(true),
        onClear:  () => { repStore.clear(); setRepBy({}); },
      }),
      intelTab === 'buy'     && h(BuyListView,   { rows: diagRows.rows, ctx: diagRows.ctx }),
  ...
  return h('div', null,
      ...
      h('div', { className: 'sticky-row' },
        h('div', { style: { flex: 1 } },
          h('div', { className: 'sticky-sub', style: { display: 'flex', alignItems: 'center', gap: 6 } },
            ...
            h('span', { style: { marginLeft: 4, fontSize: 9, color: '#3d5080', fontWeight: 600 } }, 'v6.25.0')),
    ...
    h('div', { className: 'main' + (view === 'intel' ? ' view-intel' : '') },
      ...
      // Reputation import — file or pasted table, no credentials required.
      repOpen && h(RepImport, {
        onClose: () => setRepOpen(false),
        onDone: (res) => { setRepBy(res.result.byPhone); setRepOpen(false); },
      }),

      // History back-fill — writes snapshots only, never touches the live pool.
      bfOpen && h(BackfillImport, {
        onClose: () => setBfOpen(false),
        onDone: (res) => { setSnaps(res.snaps); setBfOpen(false); },
      }),
